...
| `ANALYSIS_REDIS_STREAM_ENABLED` | `true` | 백엔드 Redis Stream publisher/consumer 활성화 여부 |
| `ANALYSIS_RESULT_CONSUMER_GROUP` | `backend-analysis-result-workers` | 백엔드가 결과 Stream을 소비하는 consumer group |
| `ANALYSIS_RESULT_CONSUMER_NAME` | `voice-backend` | 백엔드 consumer name |
| `ANALYSIS_RESULT_POLL_COUNT` | `10` | 한 번에 읽을 결과 메시지 수 |
| `ANALYSIS_RESULT_POLL_DELAY_MILLIS` | `1000` | 결과 Stream polling 간격 |
| `ANALYSIS_RESULT_PENDING_TIMEOUT_MILLIS` | `300000` | pending 결과 메시지를 claim하기 전 최소 idle 시간 |
...
XADD analysis:result * \
  ...
  failureReason null \
  segments '[{"sequenceNo":1,"expectedText":"안녕","recognizedText":"안녕","startMs":0,"endMs":700,"matchType":"MATCH","resultStatus":"NORMAL","targetUnit":"FINAL_CONSONANT_NG","errorType":null,"pronunciationScore":91.2,"intonationScore":86.0,"feedback":"받침 발음이 명확합니다."}]'
```
...
4. 백엔드는 DB transaction commit 이후 `analysis:request` Stream에 메시지를 발행한다.
...
- 요청 발행 구현체: `training/infrastructure/RedisAnalysisJobPublisher`
- 결과 수신 구현체: `analysis/infrastructure/redis/AnalysisResultStreamConsumer`
- 결과 저장 유스케이스: `analysis/application/AnalysisResultStreamService`
- 개발/테스트용 mock publisher: `analysis.redis-stream.enabled=false`일 때 `training/infrastructure/MockAnalysisJobPublisher` 사용

백엔드는 `analysis.redis-stream.enabled=true`일 때 다음 동작을 수행한다.

1. `TrainingAnalysisRequestService`가 선택 녹음, 음질, 중복 분석 조건을 검증한다.
2. `analysis_results`에 `PENDING` row를 생성한다.
3. 선택 녹음의 `audioUrl`, 콘텐츠 `scriptText`, 세션 `learningFocus`, `userId`를 조회해 transaction commit 이후 `analysis:request` Stream에 발행한다.
4. `AnalysisResultStreamConsumer`가 `analysis:result` Stream을 consumer group으로 읽는다.
5. 메시지 저장 성공 시에만 ACK한다.
6. 메시지 파싱 또는 DB 저장 실패 시 ACK하지 않아 Redis pending entry로 남긴다.
7. consumer는 설정된 idle 시간 이후 pending 메시지를 claim해 재처리한다.
8. 이미 terminal 상태인 분석 결과에 늦게 도착한 `PROCESSING`, `COMPLETED`, `FAILED` 메시지는 상태를 되돌리지 않고 ACK 처리한다.

`segments` 필드는 Redis Stream field 값 하나에 JSON array 문자열로 담는다. 값이 없거나 빈 문자열이면 빈 목록으로 처리한다.
...
백엔드는 `PENDING -> PROCESSING -> COMPLETED/FAILED` 흐름만 상태 변경으로 반영한다. `COMPLETED`, `FAILED` 같은 terminal 상태에 늦게 도착한 stale 메시지는 무시한다.
...
현재 백엔드 consumer는 새 메시지를 consumer group으로 읽고 처리 성공 시 ACK한다. ACK되지 않은 `analysis:result` pending 메시지는 `ANALYSIS_RESULT_PENDING_TIMEOUT_MILLIS` 이후 현재 백엔드 consumer가 claim해 재처리한다. retry exceeded 처리는 별도 운영 worker에서 이 정책에 맞춰 확장한다.
