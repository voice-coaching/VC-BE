...
public class OnboardingController {
    ...
    @PutMapping("/me")
    public ApiResponse<OnboardingResponseDto> saveMyOnboarding(@RequestBody OnboardingSaveRequestDto request) {
        OnboardingResponseDto response = onboardingService.saveMyOnboarding(request);
        return ApiResponse.success("온보딩이 완료되었습니다.", response);
    }

    @GetMapping("/me")
    public ApiResponse<OnboardingDetailResponseDto> getMyOnboarding() {
        OnboardingDetailResponseDto response = onboardingService.getMyOnboarding();
        return ApiResponse.success("온보딩 정보를 조회했습니다.", response);
    }

    @PatchMapping("/me")
    public ApiResponse<OnboardingPatchResponseDto> updateMyOnboarding(@RequestBody OnboardingUpdateRequestDto request) {
        OnboardingPatchResponseDto response = onboardingService.updateMyOnboarding(request);
